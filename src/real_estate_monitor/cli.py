...
from real_estate_monitor.web import serve_dashboard, serve_report_portal
...
def build_parser() -> argparse.ArgumentParser:
    ...
    report_portal = subparsers.add_parser("report-portal", help="Start the client-facing report request website")
    report_portal.add_argument("--host", default="127.0.0.1")
    report_portal.add_argument("--port", type=int, default=8001)
...
async def _run(args: argparse.Namespace) -> int:
    ...
    if args.command == "report-portal":
        serve_report_portal(settings, host=args.host, port=args.port)
        return 0
...
def _print_safe_config(settings: Settings) -> None:
    ...
    print(f"Dashboard separate users: {len(settings.web_user_credentials)}")
