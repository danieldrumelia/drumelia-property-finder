...
class Settings:
    ...
    web_users: str | None
    custom_report_max_concurrent_jobs: int
    ...
    def from_env(cls) -> "Settings":
        ...
        return cls(
            ...
            web_users=os.getenv("WEB_USERS") or None,
            custom_report_max_concurrent_jobs=int(os.getenv("CUSTOM_REPORT_MAX_CONCURRENT_JOBS", "2")),
        )
    ...
    def valid_web_credentials(self, username: str, password: str) -> bool:
        if not self.web_auth_enabled:
            return True
        users = self.web_user_credentials
        if users:
            stored_password = users.get(username)
            return stored_password is not None and secrets.compare_digest(password, stored_password)
    ...
    @property
    def web_user_credentials(self) -> dict[str, str]:
