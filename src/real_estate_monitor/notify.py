        ...
        with smtplib.SMTP(self.smtp_host, self.smtp_port, timeout=30) as smtp:
            ...
            for recipient in self.recipients:
                ...
                try:
                    ...
                    logger.warning("SMTP failed for email recipient %s: %s", _mask_email(recipient), exc)
                    continue
                if refused:
                    failed_recipients.append(recipient)
            ...
            if sent_count == 0 and failed_recipients:
                raise RuntimeError("SMTP did not accept any email recipients")
            if failed_recipients:
                logger.warning("Email sent to %s recipient(s); %s recipient(s) failed", sent_count, len(failed_recipients))
