...
from real_estate_monitor.custom_reports import build_and_send_custom_report
...
LOGO_PATH = ASSET_DIR / "drumelia-mark.png"
...
_CUSTOM_JOBS: dict[str, CustomReportJob] = {}
_CUSTOM_JOBS_LOCK = threading.Lock()
_PORTAL_SESSIONS: dict[str, str] = {}
_PORTAL_SESSION_COOKIE = "drumelia_report_session"
...
def serve_report_portal(
    settings: Settings,
    host: str = "127.0.0.1",
    port: int = 8001,
) -> None:
    handler = _make_report_portal_handler(settings)
    server = ThreadingHTTPServer((host, port), handler)
    print(f"Report portal running at http://{host}:{port}")
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print("\nReport portal stopped.")
    finally:
        server.server_close()


def _make_report_portal_handler(settings: Settings) -> type[BaseHTTPRequestHandler]:
    class ReportPortalHandler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:
            path = urlparse(self.path).path
            if path == "/health":
                self._send_json({"ok": True})
                return
            if path == "/assets/drumelia-mark.png":
                _send_logo_asset(self)
                return
            if path == "/login":
                if self._is_authorized():
                    self._redirect("/")
                    return
                self._send_html(_portal_login_page())
                return
            if path == "/logout":
                self._clear_session()
                return
            if not self._is_authorized():
                ...
                return
            if path in {"", "/"}:
                self._send_html(_report_portal_page())
                return
            if path == "/api/custom-report/jobs":
                self._send_json(_custom_jobs_payload(recipient=self._current_username()))
                return
        ...
        def do_POST(self) -> None:
            ...
            if path == "/login":
                ...
                if settings.valid_web_credentials(username, password):
                    self._redirect("/", cookie=self._start_session(username))
                    return
                self._send_html(_portal_login_page(error="Incorrect email or password"), status=HTTPStatus.UNAUTHORIZED)
                return
            if not self._is_authorized():
                self._send_json({"ok": False, "error": "Login required"}, status=HTTPStatus.UNAUTHORIZED)
                return
            if path == "/api/custom-report":
                form = self._read_form()
                query = form.get("query", [""])[0].strip()
                recipient = self._current_username()
                if not query:
                    self._send_json({"ok": False, "error": "Search is required"}, status=HTTPStatus.BAD_REQUEST)
                    return
                if not recipient:
                    self._send_json({"ok": False, "error": "Login required"}, status=HTTPStatus.UNAUTHORIZED)
                    return
                job = _start_custom_report_job(query, recipient, settings)
                self._send_json({"ok": True, "jobId": job.job_id})
                return
            self.send_error(HTTPStatus.NOT_FOUND, "Page not found")
        ...
        def _start_session(self, username: str) -> str:
            token = secrets.token_urlsafe(32)
            _PORTAL_SESSIONS[token] = username
            return f"{_PORTAL_SESSION_COOKIE}={token}; HttpOnly; SameSite=Lax; Path=/"
        ...
        def _clear_session(self) -> None:
            ...
            self.send_header(
                "Set-Cookie",
                f"{_PORTAL_SESSION_COOKIE}=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0",
            )
            self.end_headers()
        ...
        def _session_token(self) -> str | None:
            ...
            for item in cookies.split(";"):
                ...
                if name == _PORTAL_SESSION_COOKIE:
                    return value
            return None
        ...
        def _current_username(self) -> str | None:
            ...
            if not token:
                return None
            return _PORTAL_SESSIONS.get(token)
    ...
    class DashboardHandler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:
            ...
            if path in {"", "/"}:
                ...
                return
            if path == "/custom-report":
                self._send_html(_custom_report_page())
                return
            if path == "/api/jobs":
                self._send_json(_jobs_payload())
                return
            if path == "/api/custom-report/jobs":
                self._send_json(_custom_jobs_payload())
                return
        ...
        def do_POST(self) -> None:
            ...
            if path == "/api/custom-report":
                form = self._read_form()
                query = form.get("query", [""])[0].strip()
                recipient = form.get("email", [""])[0].strip()
                if not query or not recipient:
                    self._send_json({"ok": False, "error": "Search and email are required"}, status=HTTPStatus.BAD_REQUEST)
                    return
                if "@" not in recipient or "." not in recipient:
                    self._send_json({"ok": False, "error": "Enter a valid email address"}, status=HTTPStatus.BAD_REQUEST)
                    return
                job = _start_custom_report_job(query, recipient, settings)
                self._send_json({"ok": True, "jobId": job.job_id})
                return
...
def _custom_report_page() -> str:
    return _page(
        title="Custom Report",
        active_site="custom-report",
        content=f"""
        <section class="page-head">
          <div>
            <p class="eyebrow">Custom Search</p>
            <h1>Request a Property Report</h1>
          </div>
          <a class="button secondary" href="/">Dashboard</a>
        </section>

        <section class="job-strip" id="job-strip" hidden></section>
        <section class="job-strip" id="custom-job-strip" hidden></section>

        <section class="panel report-builder">
          <div class="panel-head">
            <h2>Search Brief</h2>
            <span>Scans {len(available_sites())} monitored sites</span>
          </div>
          <form class="report-form" id="custom-report-form">
            <label>
              <span>What should we find?</span>
              <input
                name="query"
                type="search"
                autocomplete="off"
            ...
            <div class="form-actions">
              ...
              <p class="muted">The email will include only properties matching the search.</p>
            </div>
          </form>
        </section>
        """,
...
def _portal_login_page(error: str | None = None) -> str:
...
<html lang="en">
  <head>
    ...
    <title>Login · Drumelia Report Portal</title>
    ...
    <main class="portal">
      <section class="login-card">
        <div class="brand-line">
          <img class="brand-mark" src="/assets/drumelia-mark.png" alt="Drumelia">
          <span>Drumelia Real Estate</span>
        </div>
        <form class="portal-form" method="post" action="/login">
          <label>
            <span>Email</span>
...
def _report_portal_page() -> str:
    return f"""<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Drumelia Report Portal</title>
    <style>{_PORTAL_CSS}</style>
  </head>
  <body>
    <main class="portal">
      <section class="portal-shell">
        ...
        <form class="portal-form" id="custom-report-form">
          <label>
            <span>Search</span>
            <input
              name="query"
          ...
          <button class="portal-button" type="submit">Send Report</button>
        </form>
        <section class="portal-status" id="custom-job-strip" hidden></section>
      </section>
    </main>
...
def _send_logo_asset(handler: BaseHTTPRequestHandler) -> None:
  ...
  <body>
    <aside class="sidebar">
      ...
      <nav>
        ...
        <a class="{ "active" if active_site == "custom-report" else "" }" href="/custom-report">Custom Report</a>
        {nav_sites}
      </nav>
    </aside>
    <main>
...
def _start_custom_report_job(query: str, recipient: str, settings: Settings) -> CustomReportJob:
    ...
    with _CUSTOM_JOBS_LOCK:
        running_jobs = sum(1 for existing_job in _CUSTOM_JOBS.values() if existing_job.status == "running")
        if running_jobs >= settings.custom_report_max_concurrent_jobs:
            raise RuntimeError("Too many reports are already running. Please try again shortly.")
        _CUSTOM_JOBS[job_id] = job

    thread = threading.Thread(
        target=_run_custom_report_job,
        args=(job_id, query, recipient, settings),
        daemon=True,
    )
    thread.start()
    return job
...
def _run_custom_report_job(job_id: str, query: str, recipient: str, settings: Settings) -> None:
    try:
        result = asyncio.run(build_and_send_custom_report(query, recipient, settings))
        with _CUSTOM_JOBS_LOCK:
            existing = _CUSTOM_JOBS[job_id]
            _CUSTOM_JOBS[job_id] = CustomReportJob(
                job_id=job_id,
                query=query,
                recipient=recipient,
                status="done",
        ...
        with _CUSTOM_JOBS_LOCK:
            existing = _CUSTOM_JOBS.get(job_id)
            _CUSTOM_JOBS[job_id] = CustomReportJob(
                job_id=job_id,
                query=query,
                recipient=recipient,
...
def _custom_jobs_payload(recipient: str | None = None) -> dict[str, object]:
    with _CUSTOM_JOBS_LOCK:
        jobs = sorted(_CUSTOM_JOBS.values(), key=lambda job: job.started_at, reverse=True)
    if recipient:
        jobs = [job for job in jobs if job.recipient == recipient]
    jobs = jobs[:5]
    return {
        "jobs": [
            {
                ...
                "htmlPath": str(job.html_path) if job.html_path else None,
                "error": job.error,
            }
            for job in jobs
        ]
...
const customJobStrip = document.getElementById('custom-job-strip');
...
function customJobMarkup(job) {
  const running = job.status === 'running';
  const done = job.status === 'done';
  const statusText = running
    ? `Scanning all websites and preparing the email for ${job.recipient}`
    : done
      ? `Sent to ${job.recipient}. ${job.totalMatches ?? 0} matching properties found.`
      : `Failed: ${job.error || 'Unknown error'}`;
  return `
    <div class="job-card">
      <div class="job-row">
        <div>
          <div class="job-title">${escapeHtml(job.query)}</div>
          <div class="job-detail">${escapeHtml(statusText)}</div>
        </div>
        <div class="job-detail">${escapeHtml(running ? job.startedAt : job.finishedAt)}</div>
      </div>
      ${running ? '<div class="progress indeterminate"><span></span></div>' : ''}
    </div>
  `;
}

async function refreshCustomJobs() {
  if (!customJobStrip) return;
  const response = await fetch('/api/custom-report/jobs');
  const data = await response.json();
  const visibleJobs = data.jobs.filter((job) => job.status === 'running' || job.status === 'done' || job.status === 'error');
  const activeJobs = data.jobs.filter((job) => job.status === 'running');
  if (visibleJobs.length) {
    customJobStrip.hidden = false;
    customJobStrip.innerHTML = visibleJobs.map(customJobMarkup).join('');
  } else {
    customJobStrip.hidden = true;
    customJobStrip.innerHTML = '';
  }
  if (customReportForm) {
    customReportForm.querySelectorAll('input, button').forEach((control) => {
      control.disabled = activeJobs.length > 0;
    });
  }
  if (!activeJobs.length && customPollTimer) {
    clearInterval(customPollTimer);
    customPollTimer = null;
  }
}

async function submitCustomReport(event) {
  event.preventDefault();
  const formData = new URLSearchParams(new FormData(customReportForm));
  customReportForm.querySelectorAll('input, button').forEach((control) => {
  ...
  customJobStrip.innerHTML = `
    <div class="job-card">
      <div class="job-title">Starting custom report</div>
      <div class="job-detail">Preparing the scan...</div>
      <div class="progress indeterminate"><span></span></div>
    </div>
  `;
  ...
  if (!response.ok) {
    ...
    customJobStrip.innerHTML = `
      <div class="job-card">
        <div class="job-title">Could not start report</div>
        <div class="job-detail">Failed: ${escapeHtml(data.error || 'Unknown error')}</div>
      </div>
    `;
    customReportForm.querySelectorAll('input, button').forEach((control) => {
  ...
  if (!customPollTimer) {
    customPollTimer = setInterval(refreshCustomJobs, 3000);
  }
}
...
if (customReportForm) {
  customReportForm.addEventListener('submit', submitCustomReport);
  refreshCustomJobs();
}
...
.login-card {
  ...
  border-radius: 10px;
  padding: 30px;
  box-shadow: 0 18px 50px rgba(24, 24, 24, .06);
}
...
function customJobMarkup(job) {
  ...
  return `
    <div class="job-card">
      <div class="job-title">${escapeHtml(job.query)}</div>
      <div class="job-detail">${escapeHtml(statusText)}</div>
      ${running ? '<div class="progress"><span></span></div>' : ''}
    </div>
  `;
}
...
async function refreshCustomJobs() {
  ...
  customJobStrip.hidden = false;
  customJobStrip.innerHTML = customJobMarkup(latestJob);
  setFormDisabled(latestJob.status === 'running');
  if (latestJob.status !== 'running' && customPollTimer) {
    clearInterval(customPollTimer);
    customPollTimer = null;
  }
}
...
async function submitCustomReport(event) {
  ...
  customJobStrip.innerHTML = `
    <div class="job-card">
      <div class="job-title">Starting report</div>
      <div class="job-detail">Preparing the scan...</div>
